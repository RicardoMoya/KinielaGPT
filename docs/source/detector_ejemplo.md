# Análisis Paso a Paso: Detección de Sorpresas

Este documento explica el proceso completo de detección de sorpresas desde los datos en bruto hasta el resultado final, usando como ejemplo el partido **VILLARREAL - GETAFE** de la jornada 19, temporada 2026.

En este proceso se analizan 4 tipos de inconsistencias que son:

1. **Inconsistencia base:**
   - Se calcula cuando hay un favorito con una probabilidad mayor al 50%.
2. **Inconsistencia por racha:**
   - Se asigna una puntuación en función de las rachas de los equipos y se comparan.
3. **Inconsistencia por resultados históricos:** 
   - Se compara la probabilidad del favorito con la probabilidad de los historicos de ese encuentro.
4. **Inconsistencia por clasificación:** 
   - Se comparan los puestos de la clasificación. Solo se aplica si hay una diferencia mayor a 8 puestos.

Se muestran estos cálculos a partir de los datos del siguiente partido:
...
Cuando hay un favorito claro (probabilidad > 50%), se calcula una **divergencia base** que representa cuánto supera la probabilidad al umbral del 50%:
...
#### 2.3 Cálculo de Score de Divergencia
...
#### 3.2 Cálculo de Tasas Históricas
...
| **⚠️ ALERTA MEDIA** | `35 <= divergence < 50` | Contradicción notable y significativa | Sorpresa posible, considerar factores adicionales |
| **⚠️ ALERTA** | `threshold <= divergence < 35` | Contradicción detectable pero moderada | Monitorear, puede haber valor oculto |
